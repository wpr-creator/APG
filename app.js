(function () {
  ...
  function createCompletionStar(resource, unlocked) {
    const completion = loadUnitZeroCompletion();
    const check = document.createElement("button");
    check.type = "button";
    check.className = "unit-completion-star";
  ...
  function route() {
    ...
    if (["agenda", "help"].includes(routeName)) {
      location.hash = "home";
      return;
    }
    const valid = ["home", "units", "foundations", "words", "skills", "exit-ticket", "madison", "constitution-explorer", "rights-referee", "election-2026", "presidential-power", "bill-journey", "federalism-map", "founding-power", "presidents"].includes(routeName) || data.units.some(unit => unit.id === routeName);
    if (routeName === "founding-power" && unitState(data.units.find(unit => unit.id === "gov-1")) === "locked") {
      location.hash = "units";
      return;
    }
    if (["madison", "constitution-explorer", "federalism-map"].includes(routeName) && unitState(data.units.find(unit => unit.id === "gov-1")) === "locked") {
      location.hash = "units";
      return;
    }
    ...
    if (["presidential-power", "bill-journey"].includes(routeName) && unitState(data.units.find(unit => unit.id === "gov-2")) === "locked") {
      location.hash = "units";
      return;
    }
  ...
  function renderUnitDetail(id) {
    ...
    if (foundations.amendments.some(amendment => amendment[4].includes(unit.id))) {
      ...
      const label = document.createElement("strong");
      label.textContent = unit.id === "gov-3" ? "BILL OF RIGHTS · AMENDMENTS 1–10" : "AMENDMENTS FOR THIS UNIT";
      const meta = document.createElement("span");
      meta.textContent = "OPEN AMENDMENT GUIDE";
      amendmentsButton.append(label, meta);
      amendmentsButton.addEventListener("click", () => {
        amendmentFilter = unit.id === "gov-3" ? "rights" : `unit:${unit.id}`;
        renderAmendments();
  ...
  function renderElection2026() {
    ...
    propositions.innerHTML = `<div class="section-heading"><div><p class="eyebrow">STATEWIDE PROPOSITIONS</p><h2>ALL 14 PROPOSITIONS</h2></div></div>`;
    const propositionGrid = document.createElement("div");
    propositionGrid.className = "proposition-grid";
    electionData.propositions.forEach(proposition => {
      ...
      propositionGrid.appendChild(details);
    });
    propositions.appendChild(propositionGrid);
